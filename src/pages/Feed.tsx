...
import { Loader, Plus, Trash2, Edit2, Image as ImageIcon, Layers, Settings, ChevronRight, X, Save, Sparkles } from "lucide-react";
...
export function Feed() {
  ...
  const handleDeleteConfig = async (id: string) => {
    if (window.confirm("Atenção: Excluir uma configuração deletará todos os seus banners e seções. Continuar?")) {
      try {
  ...
  const openBannerModal = (banner?: any) => {
    if (banner) {
      ...
      setBannerForm({
        ...
        feed_config_id: banner.feed_config_id || (data.configurations[0]?.id || ""),
        image: null,
  ...
  const handleSaveBanner = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingBanner) {
        await api.updateFeedBanner(editingBanner.id, payload, image || undefined);
  ...
  const openSectionModal = (section?: any) => {
    if (section) {
      ...
      setSectionForm({
        ...
        feed_config_id: section.feed_config_id || (data.configurations[0]?.id || ""),
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        >
          ...
          {activeTab === "banners" ? "Novo Banner" : activeTab === "sections" ? "Nova Seção" : "Nova Configuração"}
        </button>
      </div>
      ...
      <div className="flex gap-2 p-1 bg-neutral-100 rounded-2xl w-fit">
        <button
          ...
          className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === "banners"
        ...
        <button
          ...
          className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === "sections"
            ? "bg-white text-neutral-950 shadow-sm"
            : "text-neutral-500 hover:text-neutral-700"
            }`}
        >
        ...
        <button
          ...
          className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all ${activeTab === "configurations"
            ? "bg-white text-neutral-950 shadow-sm"
            : "text-neutral-500 hover:text-neutral-700"
            }`}
        >
          ...
          <div className="divide-y divide-neutral-50">
            {data.banners.length === 0 ? (
              ...
              data.banners.map((banner) => (
                <div key={banner.id} className="p-4 flex items-center gap-4 hover:bg-neutral-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-neutral-950 truncate">{banner.title || "Sem título"}</h4>
                    <p className="text-sm text-neutral-500 truncate">{banner.subtitle || "Sem subtítulo"}</p>
                    <div className="flex items-center gap-3 mt-1">
                      <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${banner.is_active ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                        {banner.is_active ? 'Ativo' : 'Inativo'}
                      </span>
                      <span className="text-[10px] text-neutral-400 uppercase font-bold">Ordem: {banner.display_order}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
          ...
          <div className="divide-y divide-neutral-50">
            {data.sections.length === 0 ? (
              ...
              data.sections.map((section) => (
                <div key={section.id} className="p-4 flex items-center gap-4 hover:bg-neutral-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-neutral-500 truncate">{section.section_type}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
        ...
        {activeTab === "configurations" && (
          <div className="divide-y divide-neutral-50">
            {data.configurations.length === 0 ? (
              <div className="p-12 text-center">
                <p className="text-neutral-400">Nenhuma configuração cadastrada.</p>
              </div>
            ) : (
              data.configurations.map((config) => (
                <div key={config.id} className="p-4 flex items-center gap-4 hover:bg-neutral-50 transition-colors">
                  <div className="w-12 h-12 bg-neutral-100 rounded-xl flex items-center justify-center text-neutral-500 shrink-0">
                    <Settings className="w-6 h-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-neutral-950 truncate">{config.name}</h4>
                    <p className="text-sm text-neutral-500 truncate">{config.is_active ? 'Configuração Ativa' : 'Inativa'}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
            ...
            <form onSubmit={handleSaveBanner} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-sm font-bold text-neutral-700 mb-1">Configuração de Feed</label>
                  <select
                    value={bannerForm.feed_config_id}
                    onChange={(e) => setBannerForm({ ...bannerForm, feed_config_id: e.target.value })}
                ...
                <div className="flex items-end pb-2">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    />
                    <span className="text-sm font-bold text-neutral-700 group-hover:text-neutral-900 transition-colors">Banner Ativo</span>
                  </label>
                </div>
                  ...
                  <div className="mt-1 flex items-center gap-4">
                    {bannerForm.image_preview && (
                      <div className="w-24 h-16 rounded-lg overflow-hidden bg-neutral-100 shrink-0 border border-neutral-200">
                        <img src={bannerForm.image_preview} className="w-full h-full object-cover" />
                      </div>
                    )}
                    <label className="flex-1">
                      <div className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-neutral-200 rounded-xl hover:border-neutral-900 hover:bg-neutral-50 transition-all cursor-pointer group">
                        ...
                        <span className="text-xs font-bold text-neutral-500 group-hover:text-neutral-900">Clique para selecionar</span>
                      </div>
                      <input
                        type="file"
                        className="hidden"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <select
                    ...
                    onChange={(e) => setSectionForm({ ...sectionForm, feed_config_id: e.target.value })}
                    className="w-full px-4 py-2 bg-neutral-50 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-neutral-900 outline-none transition-all"
                    required
                  >
                    <option value="">Selecione...</option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSectionForm({ ...sectionForm, max_items: parseInt(e.target.value) })}
                    className="w-full px-4 py-2 bg-neutral-50 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-neutral-900 outline-none transition-all"
                    required
                  />
                </div>
                ...
                <div className="col-span-2 flex items-center gap-2 cursor-pointer group">
                  <input
                    ...
                    className="w-5 h-5 rounded border-neutral-300 text-neutral-900 focus:ring-neutral-900"
                  />
                  <label htmlFor="is_visible" className="text-sm font-bold text-neutral-700 group-hover:text-neutral-900 transition-colors cursor-pointer">Seção Visível</label>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2 cursor-pointer group">
                  <input
                    ...
                    className="w-5 h-5 rounded border-neutral-300 text-neutral-900 focus:ring-neutral-900"
                  />
                  <label htmlFor="config_active" className="text-sm font-bold text-neutral-700 group-hover:text-neutral-900 transition-colors cursor-pointer">Configuração Ativa</label>
                </div>
              </div>
