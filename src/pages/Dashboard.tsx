...
export function Dashboard() {
  ...
  const renderMarkdown = (text: string) => {
    ...
    return text.split("\n").map((line, i) => {
      // Headers
      if (line.startsWith("### "))
        return <h4 key={i} className="text-base font-bold mt-4 mb-2 text-neutral-800">{line.replace("### ", "")}</h4>;
      if (line.startsWith("## "))
        return <h3 key={i} className="text-lg font-bold mt-5 mb-2 text-neutral-900 border-b pb-1">{line.replace("## ", "")}</h3>;
      if (line.startsWith("# "))
        return <h2 key={i} className="text-xl font-bold mt-6 mb-3 text-neutral-950">{line.replace("# ", "")}</h2>;

      // List items
      if (line.trim().startsWith("- "))
        return <p key={i} className="text-sm text-neutral-600 mb-1 pl-4 relative before:content-['•'] before:absolute before:left-0 before:text-neutral-400 font-medium">{processBold(line.replace("- ", ""))}</p>;

      // Paragraphs
      if (line.trim() === "") return <div key={i} className="h-2"></div>;

      return <p key={i} className="text-sm text-neutral-600 mb-1">{processBold(line)}</p>;
    });
  };
  ...
  const processBold = (text: string) => {
    ...
    return parts.map((part, index) => {
      if (part.startsWith("**") && part.endsWith("**")) {
        return <strong key={index} className="text-neutral-900">{part.slice(2, -2)}</strong>;
      }
      return part;
    });
  };
  ...
  return (
    <div className="space-y-8 p-6 max-w-400 mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {statsItems.map((item, idx) => (
              <div
                key={item.label}
                className={`p-5 flex flex-col justify-between rounded-3xl shadow-sm border border-neutral-100 ${idx === 0
                  ? "bg-neutral-900 text-white border-neutral-800"
                  : "bg-white"
                  }`}
              >
                <div>
                  <h3 className={`text-xs font-bold uppercase tracking-wider mb-1 ${idx === 0 ? "text-neutral-400" : "text-neutral-500"}`}>
                    {item.label}
                  </h3>
                  {item.description && (
                    <span className={`text-[10px] ${idx === 0 ? "text-neutral-500" : "text-neutral-400"}`}>{item.description}</span>
                  )}
                </div>
                <div className="mt-4">
                  <p className={`text-2xl font-black ${idx === 0 ? "text-white" : "text-neutral-950"}`}>
                    {item.isCurrency
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {monitoringItems.map((item) => (
              <div key={item.label} className="bg-white p-4 rounded-3xl shadow-sm border border-neutral-100 flex items-center gap-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${item.bg} ${item.color}`}>
                  <item.icon size={24} />
                </div>
                <div>
                  <p className="text-xs font-bold text-neutral-500 uppercase tracking-wide">{item.label}</p>
        ...
        {/* Right Column: AI Summary & Top Products */}
        <div className="lg:col-span-4 space-y-6">
          {/* AI Summary Card */}
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-neutral-100 flex flex-col relative overflow-hidden group min-h-[400px]">
            <div className="space-y-4 relative z-10 flex-1 flex flex-col">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  title="Recalcular"
                >
                  <RotateCw className={`w-4 h-4 ${loadingAI ? 'animate-spin' : ''}`} />
                </Button>
              </div>
              ...
              {aiSummary ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4 py-8">
                  ...
                  <p className="text-sm text-neutral-500 max-w-[200px]">
                    Nenhum insight gerado ainda. Clique abaixo para analisar sua loja.
                  </p>
                  <Button
                    onClick={generateAI}
                    disabled={loadingAI}
          ...
          {/* Top Products Card */}
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-neutral-100 overflow-hidden">
            <h3 className="text-lg font-bold text-neutral-900 mb-4 px-2">Mais Vendidos</h3>
            ...
            {topSelling.length === 0 ? (
              ...
              <div className="space-y-4">
                {topSelling.slice(0, 5).map((item, idx) => (
                  <div key={item.product_id} className="flex items-center gap-3 p-2 hover:bg-neutral-50 rounded-xl transition-colors">
                    ...
                    <div className="w-10 h-10 rounded-lg bg-neutral-100 overflow-hidden border border-neutral-200 shrink-0">
                      {item.image_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-neutral-300"><ImageIcon size={14} /></div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-neutral-900 truncate">{item.name}</p>
                      <p className="text-xs text-neutral-500">{item.total_sold} vendas</p>
                    </div>
                  </div>
                ))}
              </div>
