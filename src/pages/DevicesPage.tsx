...
import { useEffect, useState, useCallback } from "react";
import { useApi } from "../services/api";
import { Monitor, Wifi, WifiOff, Star, Trash2, RefreshCw, ChevronDown, Save } from "lucide-react";
import { Badge } from "../components/ui/badge";
import { Button } from "../components/ui/button";
import { Layout } from "../components/Layout";
...
export function DevicesPage() {
  ...
  const savePrinterForRole = async (role: "photo" | "letter", printerName: string) => {
    setSavingPrinter(true);
    try {
      await api.savePrinterConfig(role, { printerName, isActive: true });
      toast.success(`Impressora de ${role === "photo" ? "fotos" : "cartinhas"} salva`);
    } catch {
    ...
    <Layout>
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button variant="outline" size="sm" onClick={() => { fetchDevices(); loadPrinterConfig(); }} disabled={loading}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-neutral-400 mt-0.5">
                        IP: {device.ip || "—"} • {new Date(device.lastSeenAt).toLocaleString("pt-BR", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit" })}
                      </div>
                    </div>

                    <div className="flex items-center gap-1">
                      {!device.isDefault && device.isActive && (
                        <Button variant="ghost" size="icon" className="h-8 w-8" title="Definir como padrão"
                          onClick={(e) => { e.stopPropagation(); setDefault(device.deviceId); }}>
                          <Star size={14} />
                        </Button>
                      )}
                      {!device.isActive && (
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:text-red-700" title="Remover"
                  ...
                  {isExpanded && (
                    <div className="border-t border-neutral-100 p-4 space-y-4">
                      ...
                      {Array.isArray(device.printers) && device.printers.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-neutral-500 mb-2">IMPRESSORAS CONECTADAS</p>
                      ...
                      {device.isDefault && device.isActive && (
                        <div>
                          <p className="text-xs font-semibold text-neutral-500 mb-2">CONFIGURAÇÃO DE IMPRESSORAS</p>
                          <div className="grid gap-3 sm:grid-cols-2">
                            <div className="space-y-1.5">
                              <label className="text-xs text-neutral-500">Fotos & Quadros</label>
                              <div className="flex gap-2">
                                ...
                                <Button size="sm" variant="outline" disabled={!photoPrinter || savingPrinter}
                                  onClick={() => savePrinterForRole("photo", photoPrinter)}>
                                  <Save size={14} />
                                </Button>
                              </div>
                            </div>
                            <div className="space-y-1.5">
                              <label className="text-xs text-neutral-500">Cartinhas</label>
                              <div className="flex gap-2">
                                <select
                                  className="flex-1 text-sm border rounded-lg px-3 py-1.5"
                                  value={letterPrinter}
                                ...
                                <Button size="sm" variant="outline" disabled={!letterPrinter || savingPrinter}
                                  onClick={() => savePrinterForRole("letter", letterPrinter)}>
                                  <Save size={14} />
                                </Button>
                              </div>
                            </div>
